...
function handler(event) {
  var domains = [
       'https://dev-user.share-gram.com/',
       'https://dev-agency.share-gram.com/',
       'https://dev-admin.share-gram.com/',
       'http://localhost:3000/',
       'http://192.168.10.243:3000/',
       'http://192.168.10.118:3000/',
       'http://192.168.10.8:3000/',
       'http://192.168.10.19:3000/'
   ];
   var response = event.response;
   var request = event.request;
   var domain = request.headers.referer ? request.headers.referer.value: '';
   var secFetchUser = request.headers['sec-fetch-user'] ? request.headers['sec-fetch-user'].value: '';

   if (secFetchUser == '?1' || !domains.includes(domain)) {
       var content = `
           <!DOCTYPE html>
           <html>
           <head>
           ...
           <body>
               <div class="message">
                   <h1>Access Denied</h1>
                   <p>Sorry, you do not have permission to access this content.</p>
               </div>
           </body>
           </html>
       `;
   ...
   if (url.substring(0, url.lastIndexOf('.')).endsWith("mnw32")){
       request.querystring = {
                                 width: {
                                   value: "32",
                                 },
